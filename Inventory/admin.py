...
class adminclass:
    def __init__(self, root):
        ...
        lbl_login = Label(
            self.root, text="Login", font=("goudy old style", 15), bg="white"
        ).place(x=350, y=150)
        lbl_contact = Label(
        ...
        try:
            if self.var_emp_id.get() == "":
                messagebox.showerror(
                    "Error", "Admin ID must be required", parent=self.root
                )
            else:
                ...
                if row is not None:
                    messagebox.showerror(
                        "Error",
                        "This Admin ID already assigned,try different",
                        parent=self.root,
                    )
                else:
                    cur.execute(
                        "Insert into admin (eid,login,name,email,gender,contact,dob,doj,pass,address,salary) values(?,?,?,?,?,?,?,?,?,?,?)",
                        (
                            self.var_emp_id.get(),
                            self.var_login.get(),
                    ...
                    con.commit()
                    messagebox.showinfo(
                        "Success", "Admin added Sucessfully", parent=self.root
                    )
                    self.show()
        ...
        try:
            if self.var_emp_id.get() == "":
                ...
                if row is None:
                    ...
                    cur.execute(
                        "Update admin set name=?,email=?,gender=?,contact=?,dob=?,doj=?,pass=?,utype=?,address=?,salary=? where eid=?",
                        (
                            self.var_emp_name.get(),
                            self.var_emp_email.get(),
                            self.var_emp_gender.get(),
                    ...
                    con.commit()
                    messagebox.showinfo(
                        "Success", "Admin Updated Sucessfully", parent=self.root
                    )
                    self.show()
        except Exception as ex:
            messagebox.showerror("Error", f"Error due to : {str(ex)}", parent=self.root)
            ...
            if self.var_emp_id.get() == "":
                ...
                if row is None:
                    ...
                    if op is True:
                        ...
                        cur.execute(
                            "delete from admin where eid=?", (self.var_emp_id.get(),)
                        )
                        con.commit()
                        messagebox.showerror(
                            "Delete", "Admin Deleted Sucessfully", parent=self.root
                        )
                        self.show()
                        self.clear()
